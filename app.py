# app.py (top) — ENV-based config (replace the old CONFIG dict)
import os
from typing import Optional

# Optional: load a local .env for development (install python-dotenv if you want)
...
def _int_from_env(v: Optional[str], default: Optional[int] = None) -> Optional[int]:
    if v is None or v == "":
        return default
    try:
        return int(v)
...
# Names of environment variables (set these in Render or your environment)
CONFIG = {
    "DISCORD_TOKEN": os.environ.get("DISCORD_TOKEN", "").strip(),
    "GUILD_ID": _int_from_env(os.environ.get("GUILD_ID")),              # 0 => global sync
    "ADMIN_ROLE_ID": _int_from_env(os.environ.get("ADMIN_ROLE_ID")),
    "ROBLOX_UNIVERSE_ID": _int_from_env(os.environ.get("ROBLOX_UNIVERSE_ID")),
    "ROBLOSECURITY": os.environ.get("ROBLOSECURITY", "").strip() or "",  # optional
    "TRACK_INTERVAL_SECONDS": _int_from_env(os.environ.get("TRACK_INTERVAL_SECONDS"), 5),
    "SCAN_MAX_PAGES": _int_from_env(os.environ.get("SCAN_MAX_PAGES"), 8),
    "SCAN_ITEM_LIMIT": _int_from_env(os.environ.get("SCAN_ITEM_LIMIT"), 800),
    # accept "true"/"false" etc.
    "COMMANDS_EPHEMERAL": _bool_from_env(os.environ.get("COMMANDS_EPHEMERAL"), True),
}
